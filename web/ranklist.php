<?php
	...
	if(isset($_GET['users'])){
		$theusers = $_GET['users'];
		$sql=("SELECT `user_id`,`nick`,`solved`,`submit` FROM `users` WHERE user_id like '".mysql_real_escape_string($_GET['users'])."%' ORDER BY `solved` DESC,submit,reg_time  LIMIT  " ). strval ( $rank ) . ",$page_size";
	}	
                if($scope){
                        $s="";
                        switch ($scope){
					...
					if(date('w')==0)
						$weekdiff-=1;
                                        $monday=mktime(0, 0, 0, date("m"),date("d")-$weekdiff, date("Y"))                                                            ;
...
if(!$OJ_MEMCACHE)mysql_free_result($result);
		...
		if(isset($_GET['users']))
			$sql = "SELECT count(1) as `mycount` FROM `users` WHERE user_id like'".mysql_real_escape_string($_GET['users'])."%'"; 
	    else
