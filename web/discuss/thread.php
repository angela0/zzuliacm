<?php
	...
	echo "<title>ZZULI Online Judge WebBoard</title>";
...
<div style="text-align:left;font-size:80%;float:left;"><a class='btn btn-default' href="newpost.php">New Thread</a></div>
<?php if ($isadmin){
...
<tr align=center class='toprow'>
	...
	<?php if ($row->pid!=0) echo "Problem ".$row->pid; else echo "MainBoard";?></a> >> <?php echo nl2br(htmlspecialchars(stripslashes($row->title)));?></td>
</tr>
	...
	<td>
     ...
     <div style="display:inline;text-align:left; float:left; margin:0 10px"><a href="../userinfo.php?user=<?php echo $row->author_id?>"><?php echo $row->author_id; ?> </a> @ <?php echo $row->time; ?></div>
		<div class="mon" style="display:inline;text-align:right; float:right">
			...
			<?php echo $i+1;?>#</span><br/><br/>
		</div>
		<div class=content style="text-align:left; margin:10px 30px">
			<pre>
			<?php	if ($row->status == 0) echo stripslashes(htmlspecialchars($row->content));
					else {
						...
						if ($isuser || $isadmin) echo stripslashes(htmlspecialchars($row->content));
					}
			?>
			</pre>
		</div>
...
<div style="font-size:90%; width:100%; text-align:center; margin-top: 10px"><a class='btn btn-default' href="#">Top</a>  <a class='btn btn-default' href="#">Previous Page</a>  <a class='btn btn-default' href="#">Next Page</a> </div>
<?php if (isset($_SESSION['user_id'])){?>
<div style="font-size:80%;">
<form action="post.php?action=reply" method=post>
<input type=hidden name=tid value=<?php echo $_REQUEST['tid'];?>>
<div><textarea placeholder='New Reply' name=content style="border:1px dashed #8080FF; width:700px; height:200px; margin: 10px 10px; padding:10px"></textarea></div>
