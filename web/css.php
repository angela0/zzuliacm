...
/*
$sql="SELECT * FROM 
		(select * from solution where problem_id>0 && ) solution
		left join users
...
/*
$sql="SELECT
        users.user_id,users.nick,solution.result,solution.num,solution.in_date, solution.memory, solution.time, solution.language, solution.code_length, solution.problem_id, solution.contest_id, solution.solution_id
                FROM
                        (select * from solution where solution.contest_id='$cid' and num>=0 ) solution
                left join users
                on users.user_id=solution.user_id";
...
if($OJ_MEMCACHE)
        ...
        if ($row['contest_id']>0) {
                $view_status[$i][1]= "<a href='contestrank.php?cid=".$row['contest_id']."&user_id=".$row['user_id']."#".$row['user_id']."'>".$row['nick']."</a>";
        }else{
                $view_status[$i][1]= "<a href='userinfo.php?user=".$row['user_id']."'>".$row['nick']."</a>";
        }
       ...
       if ($row['contest_id']>0) {
        ...
        if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].="    <span>". (100-$row['pass_rate']*100)."%</span>";
        if(isset($_SESSION['http_judge'])) {
       //           $view_status[$i][3].="<form method=post action=admin/problem_judge.php><input type=hidden name=sid value='".$row['solution_id']."'>";
