<html>
<head>
	<title>ZZULI Online Judge WebBoard</title>
</head>
<?php
...
<?php 
	if(isset($_GET['cid'])){
		$cid = $_GET['cid'];
		echo "<center><a href='/contest.php?cid=".$cid."'>Contest ".$cid."</a></center>"; 
	}
?>
...
if ($prob_exist){?>
	...
	<a class='btn btn-default' href="newpost.php<?php if ($pid!=0 && $cid!=null) echo "?pid=".$pid."&cid=".$cid;
	else if ($pid!=0) echo "?pid=".$pid;
	else if ($cid!=null) echo "?cid=".$cid;?>">New Thread</a></div><br/><br/>

	<div style="font-size:80%;color:red;font-weight:bold">
	<?php if ($pid!=null && $pid!=0 && ($cid=='' || $cid==null)){?>
	<a class='btn btn-default' href="../problem.php?id=<?php echo $pid?>">See the problem</a>
	<?php }?>
	</div>
</div>
